...
function _register (name, m) {
	...
	if (Y.isObject(m)) {
		...
		Env._legacy._useQueue.add ({
			fn: function () {
				Y.log ('Registering a module: '+ m.name, m);
			...
			fn: function () {
				YUI ({
					...
					if (result.success) {
						...
						Y.log ('Error trying to include yui 2 loader in the page');
					}
				});
			},
			autoContinue: false
...
}, '@VERSION@' ,{requires:['node-base', 'get', 'async-queue'], optional:['node-base', 'get', 'async-queue']});
